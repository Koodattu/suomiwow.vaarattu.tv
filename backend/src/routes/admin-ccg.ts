...
router.post(
  "/snapshots",
  adminRoute(async () => {
    if (!ccgRaidRunner.triggerSnapshot("admin")) {
...
router.post(
  "/publications",
  adminRoute(async () => {
    if (!ccgRaidRunner.triggerPublication("admin")) {
      throw new CcgPublisherError(409, "ccg_raid_run_already_running", "A CCG snapshot or publication run is already in progress");
    }
    return { started: true };
  }),
...
router.post(
  ...
  adminRoute(async (req) => {
    ...
    return ccgPublisherService.enableSet(zoneId, new mongoose.Types.ObjectId(String(userId)), {
      ...
      expectedRevision: typeof req.body?.expectedRevision === "string" ? req.body.expectedRevision : undefined,
    });
  }),
);
