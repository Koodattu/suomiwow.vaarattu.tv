...
test("character checker uses the highest complete mechanics pull count instead of a stale tier entry", () => {
  const status = resolveCcgCharacterMechanicsStatus(
    [
      { pulls: 833, score: 72.9, parseScore: 80.8, survivalScore: 65, survivalPercentile: 65 },
      { pulls: 35, score: 72.2, parseScore: 70.4, survivalScore: 74, survivalPercentile: 74 },
    ],
    { pulls: 35, score: 72.2, parseScore: 70.4, survivalScore: 74, survivalPercentile: 74 },
  );
...
test("character checker requires the mechanics percentile", () => {
  const status = resolveCcgCharacterMechanicsStatus([
    { pulls: 45, score: 72.2, parseScore: 70.4, survivalScore: 74 },
  ]);

  assert.deepEqual(status, { pulls: 45, scoresReady: false, eligible: false });
});
