...
- Supporter cards display the guild returned by Armory, including guilds not tracked by the site. Creation and Armory refresh save the guild name/realm and link a tracked guild when available. Refresh previews guild changes; Apply changes updates the existing public card. Failed refreshes preserve the previous guild. Existing cards can pick up this information through Refresh from Armory and Apply changes; no new snapshot is created.
